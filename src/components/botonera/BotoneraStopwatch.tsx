...
export const BotoneraStopwatch: React.FC<BotoneraStopwatchProps> = ({
  ...
  const handleConfirmNextPeriod = () => {
    ...
    if (period === 1) { onPeriodChange(2); onTimerChange(2700); }
    else if (period === 2) { onPeriodChange(3); onTimerChange(5400); }
    else if (period === 3) { onPeriodChange(4); onTimerChange(6300); }
      ...
      <div className="flex flex-wrap items-center gap-2 bg-slate-950 border border-slate-800 rounded-2xl px-3 py-2.5 shadow-xl select-none">
        ...
        >
          {isTimerRunning ? (
            ...
            <><Play className="w-3.5 h-3.5 fill-slate-950" /> INICIAR [Espacio]</>
          )}
        </button>
