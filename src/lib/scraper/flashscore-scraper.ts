...
export async function scrapeFlashscoreMatches(maxMatches = 20): Promise<Match[]> {
  ...
  console.log(`Found ${matchLinks.length} match links on Flashscore.`);
  ...
  for (let i = 0; i < targetLinks.length; i++) {
    ...
    console.log(`[${i + 1}/${targetLinks.length}] Scraping detail: ${item.href}`);
    ...
    try {
      ...
      const matchData = await page.evaluate((matchHref) => {
        ...
        // 4. Liga - Jornada (span with data-testid="wcl-scores-overline-03" or breadcrumbs)
        let compText = '';
        let roundText = '';
        ...
        const scoreText = scoreElem ? (scoreElem as HTMLElement).innerText.trim().replace(/\n/g, ' ') : '';
        let homeScore = 0;
        let awayScore = 0;
